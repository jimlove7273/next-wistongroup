                    ...
                    {openCategory === category.name && (
                      <div className="ml-0 space-y-0 bg-white/50 animate-in fade-in duration-200">
                        {category.subcategories.map((subcategory) => {
                          // subcategory is an object { label, value } in `lib/products.ts`
                          const label =
                            (subcategory as any).label ?? String(subcategory);
                          const value = (subcategory as any).value ?? label;
                          ...
                          return (
                            <Link
                              ...
                              href={`/products?category=${encodeURIComponent(
                                category.name,
                              )}&subcategory=${encodeURIComponent(String(value))}`}
